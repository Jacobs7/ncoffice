            ...
            public void run() {
                ...
                try {
                    ...
                    if (StringUtils.isBlank(username)) {
                        //redis 写入错误提示 并设置redis内保存时间
                        RedisUtil.set(DAPE_UPMS_SERVER_SESSION_ID + "_" + sessionId + "_error", "userNameEmpty", (int) 3);
                    }
                    if (StringUtils.isBlank(password)) {
                        //redis 写入错误提示 并设置redis内保存时间
                        RedisUtil.set(DAPE_UPMS_SERVER_SESSION_ID + "_" + sessionId + "_error", "passwordEmpty", (int) 3);
                    }
                    ...
                    if (StringUtils.isBlank(hasCode)) {
                        ...
                        try {
                            ...
                            RedisUtil.set(DAPE_UPMS_SERVER_SESSION_ID + "_" + sessionId + "_error", ""+UpmsSession.OnlineStatus.not_exit.toString(), (int) 100);

                        } catch (IncorrectCredentialsException e) {
                            upmsSessionDao.loginfail(sessionId, UpmsSession.OnlineStatus.login_fail.toString());
                            RedisUtil.set(DAPE_UPMS_SERVER_SESSION_ID + "_" + sessionId + "_error", ""+UpmsSession.OnlineStatus.login_fail.toString(), (int) 100);

                        } catch (LockedAccountException e) {
                            upmsSessionDao.loginfail(sessionId, UpmsSession.OnlineStatus.loacked.toString());
                            RedisUtil.set(DAPE_UPMS_SERVER_SESSION_ID + "_" + sessionId + "_error", ""+UpmsSession.OnlineStatus.loacked.toString(), (int) 100);
