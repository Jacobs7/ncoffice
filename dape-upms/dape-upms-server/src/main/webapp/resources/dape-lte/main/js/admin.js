...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('dape-upms-systemid', systemid);
		$.cookie('dape-upms-systemname', systemname);
		$.cookie('dape-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('dape-upms-systemid') || 1;
	var systemname = $.cookie('dape-upms-systemname') || 'dape-upms-server';
	var systemtitle = $.cookie('dape-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
